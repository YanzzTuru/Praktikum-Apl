...
// Prosedur tampilkan data (pointer)
void tampilkanSepatuPointer(Sepatu* daftar, int* jumlah) {
    cout << "\n+-----------------------------------------------------------+\n";
...
// Prosedur tambah sepatu (pointer)
void tambahSepatuPointer(Sepatu* daftar, int* jumlah) {
    cin.ignore();
    string nama;
    int harga, stok;
...
void ubahSepatu(Sepatu daftar[], int jumlah) {
    tampilkanSepatuPointer(daftar, &jumlah);
    int pilih;
...
void sortNamaAscending(Sepatu daftar[], int jumlah) {
    for (int i = 0; i < jumlah - 1; i++) {
        for (int j = 0; j < jumlah - i - 1; j++) {
            if (daftar[j].nama > daftar[j + 1].nama) {
                Sepatu temp = daftar[j];
                daftar[j] = daftar[j + 1];
                daftar[j + 1] = temp;
            }
        }
    }
    cout << "Data telah diurutkan berdasarkan nama (A-Z).\n";
}

// Sorting harga descending
void sortHargaDescending(Sepatu daftar[], int jumlah) {
    for (int i = 0; i < jumlah - 1; i++) {
        for (int j = 0; j < jumlah - i - 1; j++) {
    ...
    cout << "Data telah diurutkan berdasarkan harga (tertinggi ke terendah).\n";
}

// Sorting stok ascending
void sortStokAscending(Sepatu daftar[], int jumlah) {
    for (int i = 0; i < jumlah - 1; i++) {
        for (int j = 0; j < jumlah - i - 1; j++) {
            if (daftar[j].stok > daftar[j + 1].stok) {
                Sepatu temp = daftar[j];
                daftar[j] = daftar[j + 1];
                daftar[j + 1] = temp;
            }
    ...
    cout << "Data telah diurutkan berdasarkan stok (terkecil ke terbesar).\n";
}
...
void menuPenjual(Sepatu* daftar, int* jumlah) {
    ...
    do {
        ...
        cout << "5. Urutkan Data Sepatu\n";
        cout << "6. Logout ke Menu Utama\n";
        cout << "Pilih menu: ";
        cin >> pilih;

        if (pilih == 1) tampilkanSepatuPointer(daftar, jumlah);
        else if (pilih == 2) tambahSepatuPointer(daftar, jumlah);
            ...
            cout << "2. Urutkan Harga (Tertinggi ke Terendah)\n";
            cout << "3. Urutkan Stok (Terkecil ke Terbesar)\n";
            cout << "Pilih jenis sorting: ";
            cin >> sortPilihan;

            if (sortPilihan == 1) sortNamaAscending(daftar, *jumlah);
            else if (sortPilihan == 2) sortHargaDescending(daftar, *jumlah);
            else if (sortPilihan == 3) sortStokAscending(daftar, *jumlah);
            else cout << "Pilihan tidak valid.\n";
        }
        else if (pilih != 6) cout << "Pilihan tidak valid.\n";
    } while (pilih != 6);
}
